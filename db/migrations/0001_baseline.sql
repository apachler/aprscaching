...
-- ============================================================================================
-- User features & the shack data plane
-- ============================================================================================
...
-- Depth
-- ============================================================================================
-- Bulletin federation, position telemetry, the raw-packet shack ring, cache
...
-- so the shack can chart speed/altitude/course over time alongside the weather series. Back-data
...
-- ─── packets recent ───
-- Raw per-station packet history for the shack. A short, hard-TTL ring of the
-- raw TNC2 frames we've heard, so an operator can inspect a station's recent traffic verbatim. This is
-- a shack-only diagnostic, NOT a long-term log — pruned aggressively by the scheduled job (cost
...
-- routing brain are pure (@aprscaching/packet); these tables back the read endpoints + sysop admin. All
-- three runtimes.
...
-- pinned shack apps, basemap choice) follow the ACCOUNT, not the browser — so signing in on a
...
-- The firehose/shack message log is TTL-pruned and scanned per station (shack.ts). Index it
