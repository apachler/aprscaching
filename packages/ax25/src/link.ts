...
export interface LinkEvents {
  ...
  /** Hand received I-frame data up to layer 3. `pid` is the frame's protocol id — a NET/ROM
   *  neighbour multiplexes network packets (PID 0xCF) and plain text on one session. */
...
export class ConnectedLink {
  ...
  private pending: { info: Uint8Array; pid?: number }[] = []; // layer-3 data not yet sent as I-frames
  ...
  private rxbuf = new Map<number, { info: Uint8Array; pid?: number }>(); // SREJ: out-of-sequence I-frames held for reassembly
  ...
  /** Queue layer-3 data to send as I-frame(s). `pid` overrides the link's default protocol id
   *  for this chunk (a NET/ROM neighbour sends network packets as PID 0xCF on the same session). */
  ...
  private onI(f: Ax25Frame): void {
    ...
    if (f.ns === this.vr) {
      ...
      this.rejSent = false;
      if (f.info) this.ev.deliver(f.info, f.pid);
      ...
      if (f.info && !this.rxbuf.has(f.ns!)) this.rxbuf.set(f.ns!, { info: f.info, pid: f.pid });
  ...
  private drainRxBuf(): void {
    while (this.rxbuf.has(this.vr)) {
      const held = this.rxbuf.get(this.vr)!;
      this.rxbuf.delete(this.vr);
      this.srejSent.delete(this.vr);
      if (held.info.length) this.ev.deliver(held.info, held.pid);
      this.vr = (this.vr + 1) % this.mod;
    }
  }
  ...
  private txI(ns: number, info: Uint8Array, pid?: number): void {
    this.ev.send({
      dst: this.remote,
      src: this.local,
      command: true,
