...
describe("ax25 frame codec", () => {
  it("round-trips a SABM command (P=1)", () => {
    ...
    expect(d.type).toBe("SABM");
    expect(d.command).toBe(true);
    expect(d.pf).toBe(true);
    expect(addrStr(d.dst)).toBe("OE8XBM-7");
    expect(addrStr(d.src)).toBe("OE8APR-1");
  });

  it("round-trips a UA response (F=1)", () => {
    const d = roundtrip({ dst: A, src: B, command: false, type: "UA", pf: true });
    expect(d.type).toBe("UA");
    expect(d.command).toBe(false);
    expect(d.pf).toBe(true);
  });
  ...
  it("round-trips an I-frame with N(S)/N(R), PID and payload", () => {
    ...
    expect(d.type).toBe("I");
    expect(d.ns).toBe(3);
    expect(d.nr).toBe(5);
    expect(d.pid).toBe(0xf0);
  ...
  it("round-trips a modulo-128 extended I-frame (7-bit N(S)/N(R), 2-octet control)", () => {
    ...
    const bytes = encodeFrame(
      { dst: B, src: A, command: true, type: "I", pf: true, ns: 100, nr: 90, pid: 0xf0, info },
      true,
    );
    const d = decodeFrame(bytes, true)!;
    expect(d.type).toBe("I");
    expect(d.ns).toBe(100);
    expect(d.nr).toBe(90);
    expect(d.pf).toBe(true);
  ...
  it("round-trips a modulo-128 extended SREJ with a 7-bit N(R)", () => {
    const d = decodeFrame(
      encodeFrame({ dst: B, src: A, command: false, type: "SREJ", pf: false, nr: 127 }, true),
      true,
    )!;
    expect(d.type).toBe("SREJ");
    expect(d.nr).toBe(127);
    expect(d.extended).toBe(true);
  });
