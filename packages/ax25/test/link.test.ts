...
describe("ax25 connected-mode link", () => {
  ...
  it("recovers an out-of-sequence frame via REJ", () => {
    ...
    let dropped = false;
    h.pump((to, f) => {
      if (!dropped && to === "B" && f.type === "I" && f.ns === 1) {
        dropped = true;
        return true;
      }
      return false;
...
describe("ax25 modulo-128 (SABME) + SREJ", () => {
  it("connects with SABME and reports the extended modulus on both ends", () => {
    ...
    expect(h.a.state).toBe("connected");
    expect(h.b.state).toBe("connected");
    expect(h.a.extended).toBe(true);
    expect(h.b.extended).toBe(true);
  ...
  it("round-trips every frame through the extended WIRE codec (encode/decode, extended)", () => {
    ...
    a = new ConnectedLink(
      ...
      B,
      {
        send: (f) => queue.push({ to: "B", bytes: encodeFrame(f), ext: !!f.extended }),
        deliver: (i) => got.A.push(dec.decode(i)),
        state: () => {},
      },
      { modulo: 128, window: 8 },
      () => clk.t,
    ...
    b = new ConnectedLink(
      ...
      A,
      {
        send: (f) => queue.push({ to: "A", bytes: encodeFrame(f), ext: !!f.extended }),
        deliver: (i) => got.B.push(dec.decode(i)),
        state: () => {},
      },
      { modulo: 128, window: 8 },
      () => clk.t,
    ...
    const pump = () => {
      let n = 0;
      while (queue.length && n++ < 2000) {
        const { to, bytes, ext } = queue.shift()!;
        (to === "A" ? a : b).onReceive(decodeFrame(bytes, ext)!);
      }
    };
    a.connect();
    pump();
