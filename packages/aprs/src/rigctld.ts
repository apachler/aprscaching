...
/**
 * rigctld.ts — a pure client for Hamlib's `rigctld` TCP text protocol (docs/21 Backend B). This is the
...
export function rigctldSetFreq(hz: number): string { return `F ${Math.round(hz)}\n`; }
/** Build a get-frequency line: `f` (reply is the Hz value). */
export function rigctldGetFreq(): string { return "f\n"; }
/** Build a set-mode line: `M <MODE> <passbandHz>`. Passband 0 = rig default. */
export function rigctldSetMode(mode: string, passbandHz = 0): string { return `M ${mode.toUpperCase()} ${Math.round(passbandHz)}\n`; }
/** Build a get-mode line: `m` (reply is MODE then passband on the next line). */
export function rigctldGetMode(): string { return "m\n"; }
/** Build a set-PTT line: `T <0|1>`. Keying is H5-gated at the call site, never here. */
export function rigctldSetPtt(on: boolean): string { return `T ${on ? 1 : 0}\n`; }
/** Build a get-PTT line: `t` (reply is 0/1). */
export function rigctldGetPtt(): string { return "t\n"; }
/** Ask the daemon to enumerate the connected rig's capabilities: `\dump_state`. */
export function rigctldDumpState(): string { return "\\dump_state\n"; }

export interface RprtResult { ok: boolean; code: number }
 ...
 */
export interface RigctldTransport { send(line: string): Promise<string> }

export class RigctldClient {
  constructor(private tx: RigctldTransport) {}
  async setFrequency(hz: number): Promise<RprtResult> { return parseRprt(await this.tx.send(rigctldSetFreq(hz))); }
  async getFrequency(): Promise<number | null> { return parseFreqReply(await this.tx.send(rigctldGetFreq())); }
  async setMode(mode: string, passbandHz = 0): Promise<RprtResult> { return parseRprt(await this.tx.send(rigctldSetMode(mode, passbandHz))); }
  async getMode(): Promise<{ mode: string; passbandHz: number } | null> { return parseModeReply(await this.tx.send(rigctldGetMode())); }
  /** PTT keying — the CALLER must H5-gate (verified callsign) before invoking this. */
  async setPtt(on: boolean): Promise<RprtResult> { return parseRprt(await this.tx.send(rigctldSetPtt(on))); }
  async getPtt(): Promise<boolean | null> { const v = (await this.tx.send(rigctldGetPtt())).trim().split(/\r?\n/)[0]; return v === "1" ? true : v === "0" ? false : null; }
}
