...
export function modulateAfsk1200(
  frame: Uint8Array,
  sampleRate = 48000,
  opts: { flags?: number; amplitude?: number } = {},
): Float32Array {
  ...
  const flagBits = (): number[] => {
    const o: number[] = [];
    for (let i = 0; i < 8; i++) o.push((0x7e >>> i) & 1);
    return o;
  };
  const bits: number[] = [];
  ...
  for (const bit of bits) {
    ...
    const start = Math.round(idx * spb),
      end = Math.round((idx + 1) * spb);
    const dph = (2 * Math.PI * tone) / sampleRate;
    for (let s = start; s < end; s++) {
      out[s] = amp * Math.sin(phase);
      phase += dph;
...
class Hdlc {
  ...
  rx(bit: number): void {
    ...
    if (this.ones === 6) {
      this.ones = 0;
      this.bits.length = Math.max(0, this.bits.length - 7);
      this.finish();
      this.bits.length = 0;
      return;
    } // flag
  ...
  private finish(): void {
    ...
    for (let i = 0; i < bytes.length; i++) {
      let v = 0;
      for (let j = 0; j < 8; j++) v |= this.bits[i * 8 + j]! << j;
      bytes[i] = v;
    }
...
export class Afsk1200Rx {
  ...
  constructor(
    ...
    this.cm = new Float32Array(this.win);
    this.sm = new Float32Array(this.win);
    this.cs = new Float32Array(this.win);
    this.ss = new Float32Array(this.win);
    for (let n = 0; n < this.win; n++) {
      this.cm[n] = Math.cos((2 * Math.PI * MARK * n) / sampleRate);
      this.sm[n] = Math.sin((2 * Math.PI * MARK * n) / sampleRate);
      this.cs[n] = Math.cos((2 * Math.PI * SPACE * n) / sampleRate);
      this.ss[n] = Math.sin((2 * Math.PI * SPACE * n) / sampleRate);
    }
