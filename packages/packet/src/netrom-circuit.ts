...
export class NetromCircuit {
  ...
  private t1Ms: number; private n2: number;
  ...
  private armFresh(): void { this.t1 = this.clock() + this.t1Ms; this.rc = 0; }  // new activity: fresh deadline + retries
  private rearm(): void { this.t1 = this.clock() + this.t1Ms; }                   // after a retransmit (rc already bumped)
  private disarm(): void { this.t1 = 0; this.rc = 0; }

  /** Drive the retransmit timer — the host calls this on a ~1 s interval (like the AX.25 link). */
  poll(): void {
    if (!this.t1 || this.clock() < this.t1) return;
    if (this.rc >= this.n2) { this.disarm(); this.to("disconnected"); return; }  // gave up → tear down + free the object
    this.rc++;
    if (this.state === "connecting" && this.connReqInfo) {
      this.tx({ circuitIndex: this.myIndex, circuitId: this.myId, txSeq: 0, rxSeq: 0, opcode: NrOp.ConnReq, flags: 0 }, this.connReqInfo);
      ...
      for (const s of this.sent)                        // resend the whole unacked window
        this.tx({ circuitIndex: this.yourIndex, circuitId: this.yourId, txSeq: s.seq, rxSeq: this.vr, opcode: NrOp.Info, flags: s.more ? NR_MORE : 0 }, s.info);
    }
    this.rearm();
  }
  ...
  private onConnAck(tp: NrTransport, info: Uint8Array): void {
    ...
    if (tp.opcode & 0x80) { this.disarm(); this.to("disconnected"); return; } // refuse (defensive; opcode masked to nibble upstream)
  ...
  private onDiscAck(): void { if (this.state === "disconnecting") { this.disarm(); this.to("disconnected"); } }
  ...
  private onInfoAck(tp: NrTransport): void {
    ...
    if (this.sent.length) this.armFresh();               // progress made, more still outstanding → reset the timer
