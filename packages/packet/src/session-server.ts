...
export interface Service {
  ...
  /** Consume NET/ROM network packets (I-frames with PID 0xCF) arriving on a session to this
   *  service. Neighbour nodes (BPQ, TheNet lineage) send routing broadcasts and L4 circuit
   *  traffic over the connected inter-node link; `sendNetrom` answers on the same session. */
  onNetrom?: (packet: Uint8Array, remote: Ax25Address, sendNetrom: (packet: Uint8Array) => void) => void;
}
...
export class SessionServer {
  ...
  onFrame(f: Ax25Frame): void {
    ...
    // AX.25 v2.2 initiators (BPQ) open with an XID negotiation probe. We negotiate nothing —
    // answer DM like a plain v2.0 station, which makes the peer mark us non-2.2 and dial
    // with a plain SABM. Silence is not an option: BPQ retries the XID N2 times and then
    // abandons the connect without ever falling back.
    if (f.type === "XID" && f.command) {
      this.o.send({ dst: f.src, src: svc.addr, command: false, type: "DM", pf: f.pf });
      return;
    }
  ...
  private open(svc: Service, sabm: Ax25Frame, key: string): void {
    ...
    const wire = (app: LineApp) => {
      ...
      slot.link = serveApp(svc.addr, remote, app, {
        ...
        onNetrom: svc.onNetrom
          ? (packet) => svc.onNetrom!(packet, remote, (reply) => slot.link?.send(reply, PID_NETROM))
          : undefined,
        onState: (s) => {
          if (s === "disconnected") {
