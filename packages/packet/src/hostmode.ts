...
const dec = (b: Uint8Array): string => { let s = ""; for (const x of b) s += String.fromCharCode(x); return s; };
...
export function hostmodeCommand(chan: number, cmd: string): Uint8Array {
  ...
  out[0] = chan & 0xff; out[1] = 0; out.set(c, 2); out[2 + c.length] = 0;
  return out;
}
...
export function hostmodeData(chan: number, data: Uint8Array): Uint8Array {
  ...
  out[0] = chan & 0xff; out[1] = 1; out[2] = (n - 1) & 0xff; out.set(data.subarray(0, n), 3);
  return out;
}
...
const nulAt = (buf: Uint8Array, from: number): number => { for (let i = from; i < buf.length; i++) if (buf[i] === 0) return i; return -1; };
...
export function parseHostmode(buf: Uint8Array): { events: HostmodeEvent[]; rest: Uint8Array } {
  ...
  while (buf.length - off >= 2) {
    const chan = buf[off]!, type = buf[off + 1]!;
    if (type === 0) { events.push({ chan, type: 0 }); off += 2; continue; }
    if (type >= 1 && type <= 4) {
      const end = nulAt(buf, off + 2); if (end < 0) break;        // wait for the terminator
      events.push({ chan, type: type as 1 | 2 | 3 | 4, text: dec(buf.slice(off + 2, end)) });
      off = end + 1; continue;
    }
    if (type === 5) {
      const end = nulAt(buf, off + 2); if (end < 0) break;
