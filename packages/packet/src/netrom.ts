...
/**
 * netrom.ts — NET/ROM node logic: the NODES routing table (learn / best-route / quality
...
export class NodesTable {
  ...
  constructor(seed: NodeRoute[] = []) {
    for (const r of seed) this.learn(r);
  }
  ...
  list(): NodeRoute[] {
    return [...this.byDest.values()].sort((a, b) => b.quality - a.quality);
  }
  ...
  decay(factor = 0.9): void {
    for (const [k, r] of this.byDest) {
      const q = Math.floor(r.quality * factor);
      if (q <= 0) this.byDest.delete(k);
      else r.quality = q;
    }
  }
}
...
export class NodeSession {
  ...
  constructor(
    ...
    private nodeAlias = "NODE",
    private nodeCall = "N0CALL-1",
  ) {
  ...
  greeting(): string[] {
    return [
      `${this.nodeCall}:${this.nodeAlias} - APRScaching NET/ROM node`,
      `Welcome ${this.call}. ? for help.`,
      this.prompt(),
    ];
  }
  private prompt(): string {
    return this.expert ? `${this.nodeAlias}>` : `${this.call} de ${this.nodeAlias}>`;
  }
  private out(...lines: string[]): { lines: string[]; connect?: string; disconnect?: boolean } {
  ...
  handle(input: string): { lines: string[]; connect?: string; disconnect?: boolean } {
    ...
    switch (w) {
      ...
      case "BYE":
      case "Q":
        return { lines: [`73 de ${this.nodeAlias}`], disconnect: true };
      case "?":
      case "H":
      case "HELP":
        return this.out("Nodes  Routes  Connect <call>  Users  MHeard  CQ <msg>  Info  Bye");
      case "X":
        this.expert = !this.expert;
        return this.out(`Expert ${this.expert ? "on" : "off"}.`);
      case "I":
      case "INFO":
      ...
      case "NODES": {
        ...
        if (arg) {
          const hit = ns.find((n) => n.alias === arg.toUpperCase() || n.call === arg.toUpperCase());
          return this.out(hit ? `${hit.alias}:${hit.call}  quality ${hit.quality}` : `No node "${arg}".`);
        }
      ...
      case "CQ":
        return this.out(`CQ de ${this.call}: ${arg}`);
      default:
        return this.out(`Invalid command "${w}". ? for help.`);
    }
  }
}
