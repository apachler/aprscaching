...
function pair() {
  ...
  const rxA: Uint8Array[] = []; const rxB: Uint8Array[] = [];
  const a = new NetromCircuit({
    send: (p) => q.push({ to: "b", p }), deliver: (i) => rxA.push(i), state: () => {},
  }, { index: 1, id: 10 }, { user: A("OE1XYZ"), node: A("OE8NOD", 1) });
  const b = new NetromCircuit({
    send: (p) => q.push({ to: "a", p }), deliver: (i) => rxB.push(i), state: () => {},
  }, { index: 2, id: 20 });
  const pump = (guard = 5000) => { let n = 0; while (q.length && n++ < guard) { const { to, p } = q.shift()!; (to === "a" ? a : b).onPacket(p.tp, p.info); } };
  return { a, b, rxA, rxB, pump };
}

describe("NET/ROM L4 circuit (docs/29 F2)", () => {
  ...
  it("decodes the originating user/node from a ConnReq (node accepting a circuit)", () => {
    ...
    const c = new NetromCircuit({ send: (p) => { captured = p; }, deliver: () => {}, state: () => {} },
      { index: 5, id: 99 }, { user: A("OE3ABC", 7), node: A("OE8NOD", 1) });
    c.connect();
