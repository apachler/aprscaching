...
import {
  ...
  ConnectSequencer,
  type CircuitDialer,
} from "../src/netrom-connect-through.js";
...
const echoDialer: CircuitDialer = (_route, hooks) => {
  ...
  const near = new NetromCircuit(
    {
      ...
      state: (s) => {
        if (s === "disconnected") hooks.onClose();
      },
    },
    { index: 1, id: 10 },
    { user: A("OE1USR"), node: A("OE8NOD", 1) },
  );
  ...
  const pump = (g = 2000) => {
    while (q.length && g-- > 0) {
      const { to, p } = q.shift()!;
      (to === "near" ? near : far).onPacket(p.tp, p.info);
    }
  };
  near.connect(4);
  pump();
...
describe("NET/ROM connect-through", () => {
  function setup(dialer: CircuitDialer) {
    ...
    node.consume(
      encodeNodesBroadcast("FAR", [{ dest: A("OE3FAR"), alias: "FAR", neighbor: A("OE2NBR"), quality: 180 }])[0]!,
      A("OE2NBR"),
    );
    ...
    let rx = "";
    const client = new ConnectedLink(A("OE1USR"), A("OE8NOD", 1), {
  ...
  it("is ready immediately for a single-hop (direct) script and fails on a busy node", () => {
    let ready = false,
      failed = "";
    const direct = new ConnectSequencer(parseConnectScript("C DB0XYZ"), {
      send: () => {},
      onReady: () => {
        ready = true;
      },
  ...
  it("returns the user to the node when the far end disconnects", () => {
    ...
    const dialer: CircuitDialer = (_r, hooks) => {
      closeHook = hooks.onClose;
