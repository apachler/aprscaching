...
export function colouriserTool(registry = new StationRegistry()): Tool {
  return {
    manifest: { name: "monitor-colouriser", title: "Monitor colouriser", author: AUTHOR, version: v, permissions: ["monitor"], description: "Colours heard traffic by NAMES.GP station type." },
    activate(ctx) {
...
export function macroPackTool(): Tool {
  ...
  return {
    manifest: { name: "ctext-macros", title: "CTEXT macro pack", author: AUTHOR, version: v, permissions: ["command"], description: "Canned /cq /73 /qth text macros." },
    activate(ctx) {
...
export function autoResponderTool(greeting = "Welcome — this is an APRScaching auto-responder. Type H for help."): Tool {
  return {
    manifest: { name: "auto-responder", title: "Auto-responder", author: AUTHOR, version: v, permissions: ["event"], description: "Greets an incoming connect (QTEXT/PMS style)." },
    activate(ctx) {
...
export function beaconSchedulerTool(): Tool {
  return {
    manifest: { name: "beacon-scheduler", title: "Beacon scheduler", author: AUTHOR, version: v, permissions: ["command", "beacon"], description: "Schedule a periodic comment beacon (TX-gated)." },
    activate(ctx) {
...
export function decoderTools(): Tool {
  return {
    manifest: { name: "digimode-decoders", title: "PSK31 + CW decoders", author: AUTHOR, version: v, permissions: ["decoder"], description: "Decode PSK31 varicode + CW (Morse) — the audio-cache F-5 decoders." },
    activate(ctx) {
...
export function builtinTools(): Tool[] {
  return [colouriserTool(), macroPackTool(), autoResponderTool(), beaconSchedulerTool(), decoderTools()];
}
