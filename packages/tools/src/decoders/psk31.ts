...
export function psk31Demod(samples: ArrayLike<number>, sampleRate: number, opts: Psk31Opts = {}): string {
  ...
  const phases: number[] = [];
  for (let s = 0; ; s++) {
    const start = Math.floor(s * sps),
      end = Math.floor((s + 1) * sps);
    if (end > samples.length) break;
    let I = 0,
      Q = 0;
    for (let i = start; i < end; i++) {
      const t = i / sampleRate;
      I += samples[i]! * Math.cos(2 * Math.PI * carrier * t);
      Q += samples[i]! * Math.sin(2 * Math.PI * carrier * t);
    }
...
export function psk31DemodRobust(samples: ArrayLike<number>, sampleRate: number, opts: Psk31Opts = {}): string {
  ...
  for (let i = 0; i < N; i++) x[i] = samples[i]! / rms;
  ...
  let best2 = 2 * nominal,
    bestM = -1;
  for (let f = 2 * nominal - 40; f <= 2 * nominal + 40; f += 0.2) {
    const m = dftMag(f);
    if (m > bestM) {
      bestM = m;
      best2 = f;
  ...
  let bestOff = 0,
    bestE = -1;
  for (let o = 0; o < sps; o += Math.max(1, Math.floor(sps / 64))) {
    const e = energy(symbolsAt(o));
    if (e > bestE) {
      bestE = e;
      bestOff = o;
    }
  ...
  let sr = 0,
    si = 0;
  for (const zz of z) {
    sr += zz.re * zz.re - zz.im * zz.im;
    si += 2 * zz.re * zz.im;
  }
  const theta = Math.atan2(si, sr) / 2,
    ct = Math.cos(theta),
    st = Math.sin(theta);
  let bits = "";
