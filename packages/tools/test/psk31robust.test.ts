...
function modBpsk(bits: string, carrierHz: number, timingPrefix: number, noiseAmp: number): number[] {
  ...
  let phase = 0;
  const emit = (ph: number) => { const start = out.length; for (let i = 0; i < sps; i++) { const t = (start + i) / SR; out.push(Math.cos(2 * Math.PI * carrierHz * t + ph)); } };
  emit(phase);
...
describe("robust PSK31 demod — squaring carrier recovery + AGC + timing recovery (docs/28 §6)", () => {
