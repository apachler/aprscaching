...
redirect, never a sign-in origin (see `docs/design/18`).
...
## Deployment
Five topologies (`docs/design/23-deployment.md` + `deploy/`): **0** Bun single-binary desktop · **1** Pi at
...
Cross-cutting decisions are recorded in `docs/design/14-open-decisions.md` (ADR set, ACCEPTED): leaderboard
...
- Adopted map & UX features — `docs/design/11-adopted-map-features.md`:
...
- Sustainability & supporter donations — `docs/design/12-sustainability-donations.md` (free-in-full,
  ...
  (Open Collective). Monetization schema lands as migration **`0012`** (`docs/design/14`), not the `0003`
  the doc names retrospectively; `entitlements`/`api_keys` are recognition-only, never feature gates.
- Profiles — `docs/design/13-profiles.md`: a thin, opt-in ham profile (display name, locator, avatar, bio,
...
- Landing & onboarding (mechanics) — `docs/design/18-landing-and-onboarding.md`: marketing landing + the
...
- Weather stations — `docs/design/17-weather-stations.md`: APRS weather is first-class and the RX side is
  ...
  Davis/Ultimeter via Web Serial (with `docs/design/16`). Small additive `sensor_readings` migration (gust,
  split rain, luminosity, snow, source). Weather is observational — never touches the A/B/C find tiers.
- RF hardware interfacing — `docs/design/16-rf-hardware-interfacing.md`: two paths, one shared codec
...
- RF-over-internet transports — `docs/design/22-rf-over-internet-transports.md` (decision-support, RESOLVED
...
- CAT rig control & companion apps — `docs/design/21-cat-control-and-companions.md` (adopts WAAT's serial→
  WebSocket/JSON-RPC *pattern*, not its one-rig engine): one rig-control API, two backends —
  browser-direct Web Serial CAT (Kenwood/Icom-CIV/Yaesu, `docs/design/16` H6) + a **Hamlib `rigctld`
...
- Live spots & remote control — `docs/design/20-live-spots-and-remote-control.md` (reframed from POTACAT,
  ...
  adopted. Small extensions live in their home docs: Web Serial CAT one-click tune → `docs/design/16` H6;
  day/night + bearing arc + ADIF export + watchlist alerts → `docs/design/11`.
- APRS-IS identity & passcodes — `docs/design/19-aprs-is-identity.md`: the passcode verifies *nothing*
  ...
  addressable; ties to `docs/design/15` T4.2). TOCALL: `APZACG` now (self-assigned), register an `APAC…`
  ...
  gated) or direct (auto-computed passcode/LoTW, verified + opt-in, `docs/design/16` H5). RX-only = `-1`.
- Federation next level (F4–F7) — `docs/design/15-federation-next.md`: takes live federation (F0–F3) further.
  ...
  **quorum** + hardening (parallel, grid/time-bucket privacy, rate-limit), ADR-5 tombstones. F5 reach:
  ...
  registry (namespace authority), observability. Resolves docs/design/06 #2/#3/#4. New migrations after `0012`.
...
smoke suites for green status; and skim `docs/design/10`/`11`/`12` Status sections. Cover: what's built
