...
The full status of every capability lives in **`docs/design/26-v1.0-consolidated-roadmap.md`** — that's the
...
- **Owned-RF Tier A · 44net PoP · IPIP-Mesh/BGP** — genuine Tier-A corroboration needs a receiver *you*
  ...
  the 44net gateway/subnet, and mesh routing is hardware + network-ops, not code. (`docs/design/22`)
- **FBB LZHUF (B0/B1) compression** — correctness *is* byte-exact compatibility with FBB's fixed Huffman
  ...
  forwarding is fully interoperable without it. (`docs/design/29`)
...
- **Capacitor mobile shell** — reuse the web app in a native iOS/Android wrapper for USB-serial / BLE-KISS
  and background operation. A build/sign/store pipeline, not a headless code core. (`docs/design/21`)
...
- **CI depth**, additional deployment-topology exercises, and the federation push-to-hub *rendezvous relay's*
  corroboration path are tracked in `docs/design/26` / `docs/design/15` — reserved seams, opened when there's a concrete need.
