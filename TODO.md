...
The full status of every capability lives in **``** — that's the
...
- **Owned-RF Tier A · 44net PoP · IPIP-Mesh/BGP** — genuine Tier-A corroboration needs a receiver *you*
  ...
  the 44net gateway/subnet, and mesh routing is hardware + network-ops, not code. (``)
...
- **Capacitor mobile shell** — reuse the web app in a native iOS/Android wrapper for USB-serial / BLE-KISS
  and background operation. A build/sign/store pipeline, not a headless code core. (``)
