...
import { bandForHz, freqToHz, gridToLatLon, dedupeSpots, filterSpots, type Spot } from "@aprscaching/shared";
import {
  normalizePota,
  normalizeGma,
  normalizeSota,
...
const spot = (p: Partial<Spot>): Spot => ({
  id: "x",
  source: "pota",
  callsign: "OE8APR",
  lat: 47,
...
describe("spots — SOTA normalizer + summit resolution (S3)", () => {
  ...
  it("resolves missing coordinates from the summit API (cached)", async () => {
    ...
    globalThis.fetch = (async () => {
      calls++;
      return new Response(JSON.stringify({ latitude: 48.1, longitude: 14.0, name: "Test Summit" }), {
...
describe("spots — reception networks (S3): grid + PSK/DX/RBN", () => {
  it("gridToLatLon returns the square centre and rejects junk", () => {
    ...
    expect(sub.lat).toBeGreaterThan(48);
    expect(sub.lat).toBeLessThan(49);
  });

  it("normalizePsk maps the sender locator to coordinates, drops gridless reports", () => {
    const out = normalizePsk({
      receptionReport: [
        {
          senderCallsign: "oe8apr",
          senderLocator: "JN77",
          frequency: "14074000",
          mode: "FT8",
          flowStartSeconds: 1717236000,
        },
...
describe("spots — dedupe + filter (S1)", () => {
  ...
  it("filterSpots honours bbox, bands, modes, sources", () => {
    ...
    expect(
      filterSpots(all, { bbox: [14, 46, 16, 48] })
        .map((s) => s.id)
        .sort(),
    ).toEqual(["fm", "in"]);
    expect(
      filterSpots(all, { bands: ["20m"] })
        .map((s) => s.id)
        .sort(),
    ).toEqual(["in", "out"]);
    expect(filterSpots(all, { modes: ["fm"] }).map((s) => s.id)).toEqual(["fm"]);
    expect(
      filterSpots(all, { sources: ["pota"] })
        .map((s) => s.id)
        .sort(),
    ).toEqual(["in", "out"]);
  });
