          ...
          return {
            async run() {
              if (/INSERT INTO callsign_verifications/.test(sql)) {
                rows.set(String(a[0]), { callsign: a[0], challenge: a[1], account_id: a[2], attempts: 0, created_at: a[3], status: "pending" });
              } else if (/UPDATE callsign_verifications SET attempts/.test(sql)) {
                // a = [MAX, cs]
                const r = rows.get(String(a[1])); if (r) { r.attempts = (r.attempts as number) + 1; if ((r.attempts as number) >= (a[0] as number)) r.status = "failed"; }
              } else if (/INSERT INTO aprs_outbox/.test(sql)) { /* ignore */ }
              return {};
            },
            async first() {
...
const ingestReq = (body: unknown) =>
  new Request("http://gw/verify/aprs/confirm", { method: "POST", headers: { "content-type": "application/json", "x-ingest-secret": "trusted-secret" }, body: JSON.stringify(body) });
const anonReq = (path: string, body: unknown) =>
  new Request("http://gw/verify/aprs/" + path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
...
async function readCode(env: Env): Promise<string> {
  ...
  const r = await env.DB.prepare("SELECT challenge, account_id, attempts, created_at, status FROM callsign_verifications WHERE callsign = ?").bind("OE8APR").first<{ challenge: string }>();
  return r!.challenge;
}
