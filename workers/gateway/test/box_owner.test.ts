    ...
    prepare(sql: string) {
      return {
        bind(...a: unknown[]) {
          return {
            async run() {
              if (/INSERT OR IGNORE INTO boxes/.test(sql)) { if (!boxes.has(String(a[0]))) boxes.set(String(a[0]), String(a[1])); }
              return { meta: { last_row_id: 1 } };
            },
            async first() {
              if (/FROM boxes WHERE box_id/.test(sql)) { const o = boxes.get(String(a[0])); return o ? { account_id: o } : null; }
...
const asUser = (call: string, body: unknown) =>
  new Request("http://gw/api/box/b1/command", { method: "POST", headers: { "content-type": "application/json", cookie: `acs=${call}` }, body: JSON.stringify(body) });
...
describe("SR-SEC-04 — box control is owner-bound", () => {
  it("the trusted backend (ingest secret) may always enqueue", async () => {
    ...
    const req = new Request("http://gw/api/box/b1/command", {
      method: "POST", headers: { "content-type": "application/json", "x-ingest-secret": "box-secret" },
  ...
  it("an anonymous caller (no session, no secret) is rejected", async () => {
    const env = makeEnv({});
    const req = new Request("http://gw/api/box/b1/command", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "status" }),
    });
