...
import {
  snapToGrid, gridSlackM, bucketWindow, distanceBucketM, bucketTs,
  rateLimited, negCached, negStore, corroborationAuthorized, clientIp,
...
describe("corroboration privacy coarsening (F4/T1.2)", () => {
  ...
  it("widening a radius by gridSlackM guarantees a snapped center never misses a real hit", () => {
    const cell = 0.005, slack = gridSlackM(cell);
    // worst case: true center at a cell corner, real hit 150 m away on the far side
    const trueLat = 47.0, trueLon = 15.0;                  // exactly on a grid line (corner-ish)
  ...
  it("bucketWindow only ever widens the window to bucket edges", () => {
    ...
    expect(since).toBe(600); expect(until).toBe(1200);
    expect(since).toBeLessThanOrEqual(1000); expect(until).toBeGreaterThanOrEqual(1001);
  });
...
describe("corroboration abuse limits (F4/T1.2)", () => {
  it("rateLimited trips once a key exceeds its budget, and resets after the window", () => {
    const key = "ip:test-A", t0 = 1_000_000;
  ...
  it("negative memoization caches within the TTL and expires after it", () => {
    const key = "neg:test-B", t0 = 2_000_000;
  ...
  it("corroborationAuthorized is open without a secret, gated with one", () => {
    ...
    expect(corroborationAuthorized(gated, new Request("http://x", { headers: { "x-fed-secret": "nope" } }))).toBe(false);
    expect(corroborationAuthorized(gated, new Request("http://x", { headers: { "x-fed-secret": "s3cret" } }))).toBe(true);
  });
