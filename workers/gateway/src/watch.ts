...
export async function handleWatchList(req: Request, env: Env): Promise<Response> {
  ...
  const unseen =
    (
      await env.DB.prepare("SELECT COUNT(*) AS n FROM watch_alerts WHERE account_id = ? AND seen = 0")
        .bind(acct)
        .first<{ n: number }>()
    )?.n ?? 0;
...
export async function handleWatchAlerts(req: Request, env: Env): Promise<Response> {
  ...
  const rows = (
    await env.DB.prepare(
      ...
      .all<{ id: number; ts: number; seen: number }>()
  ).results;
  const page = paginate(rows, pg.limit, (r) => ({ primary: r.ts, id: r.id }));
  return json({
    alerts: page.items.map((r) => ({ ...r, seen: r.seen === 1 })),
...
export async function recordWatchHeard(env: Env, heard: { src: string; lat: number; lon: number }[]): Promise<void> {
  ...
  for (const w of watchers) {
    const recent = await env.DB.prepare(
      ...
      .bind(w.acct, w.callsign, t - HEARD_THROTTLE_SEC)
      .first();
    if (recent) continue;
    ...
    const cache = await env.DB.prepare(
      ...
      .bind(
        p.lat - NEAR_CACHE_DEG,
        p.lat + NEAR_CACHE_DEG,
        p.lon - NEAR_CACHE_DEG,
        p.lon + NEAR_CACHE_DEG,
        p.lat,
        p.lat,
        p.lon,
    ...
    const detail = cache
      ? `${w.callsign} heard near ${cache.code} — ${cache.title}`
      : `${w.callsign} heard on the network`;
    await env.DB.prepare(
