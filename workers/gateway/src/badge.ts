...
function renderBadge(d: { callsign: string; finds: number; points: number; rank: number; hides: number; instance: string }): string {
  ...
  const stat = (label: string, value: string, x: number) =>
    `<text x="${x}" y="58" font-size="22" font-weight="700" fill="#fff" font-family="system-ui,Segoe UI,Roboto,sans-serif">${value}</text>` +
    `<text x="${x}" y="76" font-size="11" fill="rgba(255,255,255,.8)" font-family="system-ui,Segoe UI,Roboto,sans-serif">${label}</text>`;
  ...
  <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
    <stop offset="0" stop-color="#2D8BAB"/><stop offset="1" stop-color="#246f88"/></linearGradient></defs>
  <rect width="${W}" height="${H}" rx="12" fill="url(#g)"/>
  <circle cx="28" cy="30" r="9" fill="#7BB912"/>
  <text x="44" y="34" font-size="16" font-weight="700" fill="#fff" font-family="system-ui,Segoe UI,Roboto,sans-serif">APRScaching</text>
  <text x="${W - 16}" y="26" text-anchor="end" font-size="20" font-weight="700" fill="#7BB912" font-family="system-ui,Segoe UI,Roboto,sans-serif">${esc(d.callsign)}</text>
  <text x="${W - 16}" y="40" text-anchor="end" font-size="10" fill="rgba(255,255,255,.7)" font-family="system-ui,Segoe UI,Roboto,sans-serif">${esc(d.instance)}</text>
