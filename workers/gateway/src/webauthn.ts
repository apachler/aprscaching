...
function cbor(buf: Uint8Array, off: number): [unknown, number] {
  const b0 = buf[off++]!; const major = b0 >> 5; const info = b0 & 0x1f;
  const len = (): number => {
    if (info < 24) return info;
    if (info === 24) return buf[off++]!;
    if (info === 25) { const v = (buf[off]! << 8) | buf[off + 1]!; off += 2; return v; }
    if (info === 26) { const v = ((buf[off]! << 24) | (buf[off + 1]! << 16) | (buf[off + 2]! << 8) | buf[off + 3]!) >>> 0; off += 4; return v; }
    throw new Error("cbor: length too large");
  };
  switch (major) {
    case 0: return [len(), off];
    case 1: { const v = len(); return [-1 - v, off]; }
    case 2: { const n = len(); const v = buf.slice(off, off + n); return [v, off + n]; }
    case 3: { const n = len(); const v = new TextDecoder().decode(buf.slice(off, off + n)); return [v, off + n]; }
    case 4: { const n = len(); const a: unknown[] = []; for (let i = 0; i < n; i++) { const [v, no] = cbor(buf, off); a.push(v); off = no; } return [a, off]; }
    case 5: { const n = len(); const map = new Map<unknown, unknown>(); for (let i = 0; i < n; i++) { const [k, n1] = cbor(buf, off); off = n1; const [v, n2] = cbor(buf, off); off = n2; map.set(k, v); } return [map, off]; }
    default: throw new Error("cbor: unsupported major " + major);
  }
}
function cborFirst(buf: Uint8Array): unknown { return cbor(buf, 0)[0]; }

// ---- authenticatorData parsing ----
type AuthData = { rpIdHash: Uint8Array; up: boolean; uv: boolean; signCount: number; credId?: Uint8Array; coseKey?: Uint8Array };
function parseAuthData(d: Uint8Array): AuthData {
  ...
  if (flags & 0x40) { // AT — attested credential data present
    ...
    const credLen = (d[off]! << 8) | d[off + 1]!; off += 2;
    out.credId = d.slice(off, off + credLen); off += credLen;
...
async function importCose(cose: Uint8Array): Promise<{ key: CryptoKey; alg: number }> {
  ...
  const kty = m.get(1);
  if (kty === 2) { // EC2 (P-256)
    const x = m.get(-2) as Uint8Array, y = m.get(-3) as Uint8Array;
    const raw = new Uint8Array(65); raw[0] = 0x04; raw.set(x, 1); raw.set(y, 33);
    return { key: await crypto.subtle.importKey("raw", raw, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"]), alg: -7 };
  }
  if (kty === 3) { // RSA
    const n = m.get(-1) as Uint8Array, e = m.get(-2) as Uint8Array;
    const jwk = { kty: "RSA", n: bytesToB64url(n), e: bytesToB64url(e), alg: "RS256", ext: true };
    return { key: await crypto.subtle.importKey("jwk", jwk, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]), alg: -257 };
  }
...
function derToRaw(der: Uint8Array): Uint8Array {
  ...
  if (der[off]! & 0x80) off += 1 + (der[off]! & 0x7f); else off += 1;
  const readInt = (): Uint8Array => {
    if (der[off++] !== 0x02) throw new Error("bad DER int");
    const n = der[off++]!; let v = der.slice(off, off + n); off += n;
...
function checkClientData(clientDataJSON: Uint8Array, type: string, challenge: string, origins: string[]): void {
  const cd = JSON.parse(new TextDecoder().decode(clientDataJSON)) as { type: string; challenge: string; origin: string };
...
export async function verifyRegistration(o: {
  clientDataJSON: Uint8Array; attestationObject: Uint8Array; challenge: string; origins: string[]; rpId: string;
...
export async function verifyAssertion(o: {
  clientDataJSON: Uint8Array; authenticatorData: Uint8Array; signature: Uint8Array;
  coseKey: string; challenge: string; origins: string[]; rpId: string; storedCounter: number;
  ...
  signed.set(o.authenticatorData, 0); signed.set(clientHash, o.authenticatorData.length);
  const ok = alg === -7
