...
const adifField = (name: string, value: string): string =>
  `<${name}:${new TextEncoder().encode(value).length}>${value}`;
...
export function findsToAdif(finds: ExpFind[], call: string): string {
  ...
  const records = finds
    .map((f) => {
      ...
      return [
        adifField("CALL", station),
        adifField("QSO_DATE", date),
        adifField("TIME_ON", time),
        adifField("MODE", "FM"),
        adifField("SIG", "APRSCACHING"),
        adifField("SIG_INFO", f.code),
        adifField("COMMENT", `Found ${f.title} (${f.code})`),
        "<EOR>",
      ].join(" ");
    })
    .join("\n");
...
function parseBbox(req: Request): [number, number, number, number] {
  const [minLon, minLat, maxLon, maxLat] = (new URL(req.url).searchParams.get("bbox") ?? "-180,-90,180,90")
    .split(",")
    .map(Number) as [number, number, number, number];
  return [minLon, minLat, maxLon, maxLat];
}
async function bboxCaches(
  env: Env,
...
export async function handleCachesKml(req: Request, env: Env): Promise<Response> {
  return download(
    cachesToKml(await bboxCaches(env, parseBbox(req))),
    "application/vnd.google-earth.kml+xml",
    "aprscaching-caches.kml",
  );
}
