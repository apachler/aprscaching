    ...
    async load(_env, scope) {
      ...
      return summits
        ...
        .map((s) => ({
          ...
          lon: num(s.longitude),
          sourceName: "SOTA",
          sourceUrl: `https://sotl.as/summits/${s.summitCode}`,
          ownerCall: "SOTA",
  ...
  pota: {
    ...
    async load(_env, scope) {
      ...
      return rows
        .filter(
          (r) =>
            r.latitude && r.longitude && (!region || (r.locationDesc ?? "").toUpperCase().split(",").includes(region)),
        )
        .map((r) => ({
          source: "pota",
          ...
          lon: num(r.longitude),
          sourceName: "POTA",
          sourceUrl: `https://pota.app/#/park/${r.reference ?? ""}`,
          ownerCall: "POTA",
      ...
      return rows
        .filter(
          (r) =>
            ...
            (!program || (r.reference ?? "").toUpperCase().startsWith(program)),
        )
        .map((r) => ({
          source: "wwff",
          externalId: r.reference ?? "",
          ...
          lon: num(r.longitude),
          sourceName: "WWFF",
          sourceUrl: r.website || `https://wwff.co/directory/`,
          ownerCall: "WWFF",
        }));
    },
  },
      ...
      return feats
        .map((f) => {
          ...
          return {
            ...
            lon: f.lon,
            sourceName: "WWBOTA",
            sourceUrl: `https://ukbota.net/atlas/?bunker=${encodeURIComponent(ref)}`,
            ownerCall: "WWBOTA",
          };
        })
  ...
  opencaching: {
    ...
    async load(env, scope) {
      ...
      for (const k of Object.keys(detail)) {
        ...
        const lat = num(String(c.location).split("|")[0]),
          lon = num(String(c.location).split("|")[1]);
        out.push({
          source: "opencaching",
          externalId: c.code ?? k,
          code: c.code ?? k,
          type: mapGeocacheType(c.type),
          title: c.name ?? k,
          lat,
          lon,
          sourceName: "OpenCaching",
    ...
    async load(_env, scope) {
      ...
      for (const w of wpts) {
        ...
        out.push({
          ...
          externalId: name,
          code: name,
          type: mapGeocacheType(w.type),
          title: w.urlname || w.desc || name,
          lat: w.lat,
          lon: w.lon,
          sourceName: "Geocaching Australia",
          sourceUrl: w.url || `https://geocaching.com.au/cache/${name}`,
          ownerCall: "GCAU",
        });
      }
      return out;
  ...
  iota: {
    ...
    async load(_env, scope) {
      ...
      for (const g of groups) {
        ...
        const lat = num(g.latitude ?? g.lat),
          lon = num(g.longitude ?? g.lon);
        ...
        out.push({
          ...
          code: ref,
          type: "traditional",
          title: g.name ?? g.groupName ?? ref,
          lat,
          lon,
          sourceName: "IOTA",
          sourceUrl: `https://www.iota-world.org/iota-groups-islands/groups.html?filter[search]=${encodeURIComponent(ref)}`,
          ownerCall: "IOTA",
        });
      }
      return out;
      ...
      return (data?.results?.bindings ?? [])
        .map((b: any) => {
          ...
          return {
            ...
            lon: num(b.lon?.value),
            sourceName: "Wikidata",
            sourceUrl: qurl || `https://www.wikidata.org/wiki/${id}`,
            ownerCall: "Wikidata",
          };
        })
