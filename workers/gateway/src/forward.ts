...
export function normalizePartner(input: unknown): ForwardPartner | null {
  ...
  const call = asStr(b.call).trim().toUpperCase();
  ...
  const msgtypes =
    [
      ...new Set(
        (asStr(b.msgtypes) || "PBT")
          .toUpperCase()
          .split("")
          .filter((c) => "PBT".includes(c)),
      ),
    ].join("") || "PBT";
  const timebands = asStr(b.timebands)
    .replace(/[^0-9,-]/g, "")
    .slice(0, 64);
  const ha = asStr(b.ha).trim();
  return {
    call,
    ha: ha ? ha.toUpperCase().slice(0, 64) : null,
    connectScript: asStr(b.connectScript).slice(0, 512),
    proto,
...
/** Map a local bbs_messages row → the FBB wire shape. Pure.
