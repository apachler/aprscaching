...
export async function handleLeaderboard(req: Request, env: Env): Promise<Response> {
  ...
  return json({ metric, period: u.searchParams.get("period") ?? "all", leaderboard: rows.map((r, i) => ({ rank: i + 1, ...r, points: Math.round(r.points) })) });
}
...
export async function handleProfile(req: Request, env: Env, callsign: string): Promise<Response> {
  ...
  ).bind(cs).first<{ finds: number; points: number; firstFind: number | null; lastFind: number | null }>();
  ...
  ).bind(cs).all<{ tier: string | null; n: number }>()).results;
  ...
  ).bind(cs).all<{ type: string; n: number }>()).results;
...
// ---------------------------------------------------------------- favorites / watches
async function toggleSet(env: Env, table: "favorites" | "watches", cacheId: number, callsign: string, on?: boolean): Promise<{ on: boolean; count: number }> {
  const cs = callsign.toUpperCase();
  const exists = await env.DB.prepare(`SELECT 1 AS x FROM ${table} WHERE callsign=? AND cache_id=?`).bind(cs, cacheId).first();
  const want = on ?? !exists;
  if (want && !exists) await env.DB.prepare(`INSERT OR IGNORE INTO ${table} (callsign, cache_id) VALUES (?,?)`).bind(cs, cacheId).run();
  if (!want && exists) await env.DB.prepare(`DELETE FROM ${table} WHERE callsign=? AND cache_id=?`).bind(cs, cacheId).run();
  const count = await env.DB.prepare(`SELECT COUNT(*) AS n FROM ${table} WHERE cache_id=?`).bind(cacheId).first<{ n: number }>();
...
export async function cacheHealth(env: Env, cacheId: number): Promise<{ needsMaintenance: boolean; dnfStreak: number; lastFound: number | null }> {
  const recent = (await env.DB.prepare(
    "SELECT log_type, ts, verified FROM cache_logs WHERE cache_id=? AND log_type IN ('found','dnf') ORDER BY ts DESC LIMIT 10",
  ).bind(cacheId).all<{ log_type: string; ts: number; verified: number }>()).results;
  let dnfStreak = 0;
  for (const r of recent) { if (r.log_type === "dnf") dnfStreak++; else break; }
...
export async function favoritesInfo(env: Env, cacheId: number, callsign?: string | null): Promise<{ favorites: number; favorited: boolean }> {
  ...
  let favorited = false;
  if (callsign) favorited = !!(await env.DB.prepare("SELECT 1 AS x FROM favorites WHERE cache_id=? AND callsign=?").bind(cacheId, callsign.toUpperCase()).first());
  return { favorites: count?.n ?? 0, favorited };
}

// ---------------------------------------------------------------- badges (awarded after a find / hide)
const FIND_BADGES: { badge: string; min: number }[] = [
  { badge: "first-find", min: 1 }, { badge: "finder-10", min: 10 }, { badge: "finder-50", min: 50 },
  { badge: "finder-100", min: 100 }, { badge: "finder-500", min: 500 },
];
const TYPE_BADGE: Record<string, string> = {
  sota: "summiteer", pota: "park-hunter", aprs_living: "rover-hunter", wwff: "flora-fauna", castle: "castle-hunter", bunker: "bunker-hunter",
};

export async function awardFindBadges(env: Env, callsign: string): Promise<void> {
  const cs = callsign.toUpperCase();
  const grant = (badge: string) => env.DB.prepare("INSERT OR IGNORE INTO achievements (callsign, badge, earned_at) VALUES (?,?,?)").bind(cs, badge, now());
  const stmts = [];
  const finds = (await env.DB.prepare("SELECT COUNT(DISTINCT cache_id) AS n FROM cache_logs WHERE logger_call=? AND log_type='found' AND verified=1").bind(cs).first<{ n: number }>())?.n ?? 0;
  for (const b of FIND_BADGES) if (finds >= b.min) stmts.push(grant(b.badge));
  if (await env.DB.prepare("SELECT 1 AS x FROM cache_logs WHERE logger_call=? AND log_type='found' AND verified=1 AND tier='A' LIMIT 1").bind(cs).first()) stmts.push(grant("rf-verified"));
  const types = (await env.DB.prepare("SELECT DISTINCT c.type FROM cache_logs l JOIN caches c ON c.id=l.cache_id WHERE l.logger_call=? AND l.log_type='found' AND l.verified=1").bind(cs).all<{ type: string }>()).results;
...
export async function awardHideBadge(env: Env, callsign: string): Promise<void> {
  const cs = callsign.toUpperCase();
  const hides = (await env.DB.prepare("SELECT COUNT(*) AS n FROM caches WHERE owner_call=? AND source='native' AND status!='archived'").bind(cs).first<{ n: number }>())?.n ?? 0;
  const grant = (badge: string) => env.DB.prepare("INSERT OR IGNORE INTO achievements (callsign, badge, earned_at) VALUES (?,?,?)").bind(cs, badge, now());
  const stmts = [];
