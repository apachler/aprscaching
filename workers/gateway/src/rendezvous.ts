...
interface LivingRow { id: number; station_call: string; lat: number | null; lon: number | null }
...
export async function recordRendezvous(env: Env, heard: { src: string; lat: number; lon: number }[]): Promise<void> {
  ...
  for (const h of heard) {
    ...
    ).bind(h.src, nowS - HEARD_WINDOW).all<LivingRow & { lastSeen: number }>()).results;
    ...
    for (const o of others) {
      ...
      ).bind(nowS - DEDUP_WINDOW, me.id, o.id, o.id, me.id).first();
      if (recent) continue;
      await env.DB.prepare(
        "INSERT INTO rendezvous_log (cache_a, cache_b, call_a, call_b, ts, lat, lon) VALUES (?,?,?,?,?,?,?)",
      ).bind(me.id, o.id, me.station_call.toUpperCase(), o.station_call.toUpperCase(), nowS, h.lat, h.lon).run();
    }
  }
}

interface RendezvousEntry { withCacheId: number; withCall: string; ts: number; lat: number | null; lon: number | null }
...
export async function rendezvousFor(env: Env, cacheId: number, limit = 10): Promise<RendezvousEntry[]> {
  ...
  ).bind(cacheId, cacheId, cacheId, cacheId, limit).all<RendezvousEntry>()).results;
  return rows;
}
