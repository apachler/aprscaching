...
const rec = (over: Partial<Parameters<typeof upsertImported>[1][number]>) => ({
  source: "sota", externalId: "X", code: "X", type: "sota" as const, title: "T",
  lat: 47.07, lon: 15.42, sourceName: "SOTA", sourceUrl: "https://sotl.as/summits/X", ...over,
});

describe("import upsert (M3)", () => {
  let env: Env, db: Database.Database;
  beforeEach(() => { ({ env, db } = freshEnv()); });

  it("inserts new imported caches with attribution, then UPDATES on re-import (no duplicate)", async () => {
    const a = await upsertImported(env, [rec({ source: "pota", externalId: "US-0001", code: "US-0001", type: "pota", title: "Acadia", sourceName: "POTA" })]);
    expect(a).toMatchObject({ imported: 1, updated: 0 });

    const b = await upsertImported(env, [rec({ source: "pota", externalId: "US-0001", code: "US-0001", type: "pota", title: "Acadia National Park", sourceName: "POTA" })]);
    expect(b).toMatchObject({ imported: 0, updated: 1 });

    const rows = db.prepare("SELECT title, source, source_name, source_url FROM caches WHERE source='pota'").all() as any[];
  ...
  it("de-duplicates across sources with ham-radio priority", async () => {
    ...
    await upsertImported(env, [rec({ source: "sota", externalId: "GM/SI-001", code: "GM/SI-001", lat: 47.0, lon: 15.0 })]);
    // an OSM peak (priority 3) ~20 m away -> should be SKIPPED (deduped, ham wins)
    const osm = await upsertImported(env, [rec({ source: "osm", externalId: "node/9", code: "OSM-9", type: "traditional", title: "Peak", sourceName: "OpenStreetMap", lat: 47.00018, lon: 15.0 })]);
  ...
  it("a higher-priority import supersedes a weaker nearby one already present", async () => {
    ...
    await upsertImported(env, [rec({ source: "osm", externalId: "node/9", code: "OSM-9", type: "traditional", title: "Peak", sourceName: "OpenStreetMap", lat: 47.0, lon: 15.0 })]);
    // then the SOTA summit at ~same spot -> inserted, OSM one archived
    const sota = await upsertImported(env, [rec({ source: "sota", externalId: "GM/SI-001", code: "GM/SI-001", lat: 47.0001, lon: 15.0 })]);
  ...
  it("does not dedupe distinct ham programs at the same spot (equal priority co-exist)", async () => {
    ...
    const wwff = await upsertImported(env, [rec({ source: "wwff", externalId: "W1", code: "W1", type: "wwff", title: "Reserve", sourceName: "WWFF", lat: 47.0, lon: 15.0 })]);
