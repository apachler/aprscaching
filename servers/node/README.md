# @aprsweb/node-gateway — portable self-host runtime
...
are shared (`@aprsweb/gateway/app`); only the bindings differ:
...
# from the repo root
pnpm install
pnpm --filter @aprsweb/node-gateway start         # http://127.0.0.1:8787  (creates ./data/aprscaching.db)
```
...
- R2 (`TILES`) is unused in M1 and stubbed.
