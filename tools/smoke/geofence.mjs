...
ws.send(
  JSON.stringify({ type: "subscribe", bbox: [LON - 1, LAT - 1, LON + 1, LAT + 1], maxAgeSec: 3600, callsign: "GEO1" }),
);
await sleep(500);
...
ok(
  "prompt carries code + distance",
  !!prompt && /^AC-/.test(prompt.code ?? "") && typeof prompt.distanceM === "number",
  JSON.stringify(prompt),
);
ok(
  "received a station delta for GEO1",
  got.some((m) => m.type === "station" && m.callsign === "GEO1"),
);
...
ok(
  "but station deltas still broadcast in-bbox",
  got.some((m) => m.type === "station" && m.callsign === "OTHER"),
);
