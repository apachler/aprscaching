...
const THEME = process.env.THEME || ""; // "phosphor" → capture the whole tour in the green-phosphor flip
...
// Open the Shack panel deterministically via the ?view=shack deep-link (the same one-shot
...
async function openShack(page) {
  const marker = () => page.locator(".shack-apps").first(); // the launcher grid (shack is a pure launcher)
  for (let attempt = 0; attempt < 2; attempt++) {
    await page.goto(`${BASE}/?view=shack&n=${++navSeq}#11.5/47.078/15.43`, { waitUntil: "load" });
    await ready(page);
    try {
  ...
  throw new Error("shack did not open");
}
...
async function launchShackApp(page, label, waitSel) {
  ...
  await page
    .locator(".shack-app-launch", { hasText: label })
    .first()
    .click()
    .catch(() => {});
...
// Enumerate the tools revealed under Profile → Advanced — the small-viewport home for Shack/BBS/
...
async function advancedTools(page) {
  ...
  await clickAny(page, [".group-toggle:has-text('Advanced')"]);
  await page.waitForTimeout(300);
  // tool buttons read like "📡 Shack": a leading glyph THEN a word. Require letters so bare-glyph
...
for (const v of VIEWS) {
  ...
  // signed-out: landing (hero + the full scrolling page) + sign-in dialog
  await step("landing", async () => {
    const ctx = await ctxFor(v, false);
    const page = await ctx.newPage();
    await page.goto(`${BASE}/`, { waitUntil: "load" });
    await page.waitForSelector(".landing", { timeout: 15000 });
    // the hero's packet-proof animation stamps its Tier-A badge at ~3 s — wait it out
    await page.waitForTimeout(3400);
    await shot(page, v.id, "landing", "Landing — hero (find verified on the air)");
    // full-page view: the landing scrolls inside its own container (the app shell is fixed-height),
    // so fullPage alone can't expand it — let the document grow for this one capture.
    await step("landing-full", async () => {
      const st = await page.addStyleTag({
        content:
          "html,body,#root,.app{height:auto!important}" + ".landing{height:auto!important;overflow:visible!important}",
      });
      seq++;
      const file = `${viewIndex(v.id)}-${String(seq).padStart(2, "0")}-${v.id}-landing-full.png`;
      await page.waitForTimeout(300);
      await page.screenshot({ path: OUT + file, fullPage: true });
      manifest.push({ file, viewport: v.id, name: "landing-full", label: "Landing — full page" });
      console.log("  ✓", file, "— Landing — full page");
  ...
  await step("shack", async () => {
    await openShack(page);
    await page.waitForSelector(".shack-apps", { timeout: 6000 }).catch(() => {});
    await page.waitForTimeout(400);
    await shot(page, v.id, "shack-launcher", "Shack — app launcher (pin to rail)");
  });
  ...
  if (process.env.TEASER_ADMIN === "1") {
    await step("node", async () => {
      await launchShackApp(page, "NET/ROM node", ".node-panel");
  ...
  await step("tools", async () => {
    await launchShackApp(page, "Tools", ".tools-panel");
