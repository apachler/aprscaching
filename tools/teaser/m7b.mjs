...
const shot = async (n) => {
  ...
  await p.screenshot({ path: new URL(`./out/${n}.png`, import.meta.url).pathname });
  console.log(n);
};
// Activity
await p.click("button:has-text('Activity')");
await p.waitForSelector(".panel.right h2");
await shot("19-activity");
// Profile
await p.click("button[title^='Profile']");
await p.waitForSelector(".panel.right h2");
await shot("20-profile");
// Nearby
await p.click("button:has-text('Nearby')");
await p.waitForSelector(".panel.right h2");
await shot("21-nearby");
// Cache detail = one-tap log: close nearby, click a pin
await p.click(".panel.right .icon");
await p.evaluate(() => {
  const e = [...document.querySelectorAll("button.cache-pin")];
  e[0]?.click();
});
await p.waitForSelector(".logform", { timeout: 8000 });
await shot("22-onetap-log");
await b.close();
