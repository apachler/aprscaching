...
const frames = fs
  .readdirSync(OUT)
  .filter((f) => /^[123]-.*\.png$/.test(f))
  .sort();
if (!frames.length) {
  console.error("no frames in", OUT);
  process.exit(1);
}
const labels = {};
for (const v of ["desktop", "tablet", "mobile"]) {
  ...
  if (fs.existsSync(f))
    for (const e of JSON.parse(fs.readFileSync(f, "utf8"))) labels[e.file] = { label: e.label, viewport: e.viewport };
}
...
const b64 = await page.evaluate(
  async ({ slides, W, H, HOLD, FADE, VPX, VPY }) => {
