...
// Workbench: Profile -> Advanced -> Workbench
...
await p.click("button:has-text('📡 Workbench')");
await p.waitForSelector(".panel.right .group");
await p.waitForTimeout(500);
await p.screenshot({ path: new URL("./out/28-workbench-grouped.png", import.meta.url).pathname });
console.log("28 done");
// expand Transports group to show toggle-gated reveal
await p.click(".group-toggle:has-text('Transports')");
await p.waitForTimeout(400);
await p.screenshot({ path: new URL("./out/29-workbench-expanded.png", import.meta.url).pathname });
console.log("29 done");
await b.close();
