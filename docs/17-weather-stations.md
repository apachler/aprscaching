...
the decoder), a **control-verified callsign**, and explicit opt-in — same gating as `docs/16` H5
(TX off by default).
...
### W4 — Browser-direct PWS via Web Serial (ties into `docs/16`)
...
- **W4 (browser-direct):** Web Serial Davis/Ultimeter/Peet-Bros decode (with `docs/16`).
