...
**APRS workbench**. You hide a cache, go find it, and log the find *verified by radio* — not just by tapping
a button. It runs in a browser, self-hosts on a Raspberry Pi, and federates with other instances into one
open network.
...
caching side is the *product*; the workbench is the *platform* it rides on.
...
| **Web app** | A React + MapLibre single-page app: the map, the workbench, and the operator surface. |
...
| **Libraries** | Pure, reusable codecs (`@aprsweb/aprs`, `@aprsweb/ax25`, `@aprsweb/packet`, `@aprsweb/tools`) and typed contracts (`@aprsweb/shared`). |
