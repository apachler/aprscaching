...
The pure CAT codec (`@aprsweb/aprs`) speaks three protocol families directly over Web Serial, so a
...
Hamlib supports. The `RigctldClient` (`@aprsweb/aprs`) speaks the `rigctld` TCP text protocol — set/get
