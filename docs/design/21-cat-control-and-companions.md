...
`docs/design/16` (RF hardware), `docs/design/20` (remote control), `docs/design/19` (APRS-IS id), `.claude/rules/*`.
...
- **Backend A — browser-direct (Web Serial CAT, `docs/design/16` H6).** Implement the common protocol
...
- **Backend B — Hamlib companion (`rigctld`).** For the **200+ rig long tail**, iOS, non-Chromium, and
  ...
  it). The companion wraps it and bridges through the gateway command channel (`docs/design/20` R1).
...
- **Gateway command channel** (`docs/design/20` R1) — the box holds an outbound authenticated connection;
  the platform sends signed beacon/message/TX-IGate/CAT commands (no inbound ports). H5-gated.
- **Easy packaging** — `docker run` / `.deb` / AppImage (the `docs/design/06` "make self-hosting trivial"
...
- **BLE-KISS** (Mobilinkd / the `docs/design/16` BLE-KISS API) — *the only* way to get RF on **iOS** (iOS
...
## 4. Platform reality (correct `docs/design/16`)
- **Web Serial:** desktop Chromium solid since Chrome 89; **Android only emerging** — Chrome 148 beta
  ...
  Treat Android Web Serial as **not yet dependable** → the companion covers it. (Update `docs/design/16`'s
...
- **TX always gated:** any keying (CAT PTT, beacon, message) requires control-verification + explicit
  opt-in (`docs/design/16` H5, `docs/design/19`); RX/tune-only by default. The companion executes only commands
...
- **C1 — rig-control API + browser CAT (Backend A):** Kenwood/Icom-CIV/Yaesu over Web Serial; one-click
  tune to APRS freq. (Pairs `docs/design/16` H6.)
...
- **C5 — remote rig control:** drive Backend B from the web app (folds into `docs/design/20` R2).
