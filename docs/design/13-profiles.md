...
- **No ads, no third-party analytics** (consistent with `docs/design/12`). One session cookie only.
...
| operated SSID stations (`OE8APR-9`…) | **deferred** — needs `account_stations` (see `docs/design/10`) | opt-in |
...
- **M5**: operated **SSID stations** on the profile, once `account_stations` lands (`docs/design/10`).
...
export/erase. Saved *map views* remain their own shareable, slug-addressed store (`docs/design/11` M1).
