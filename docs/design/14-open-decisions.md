...
open points and unblock `docs/design/11` (features) and `docs/design/12` (sustainability). Implementation notes
...
**Implementation.** `api_keys` table (lands with `docs/design/11`). Edge rate-limit (per-IP + per-key),
...
**Implementation.** `push_subs` table (with `docs/design/11`) + email-digest job + PWA-install gating.
...
| **ADR-4a/4b** API + push | `api_keys`, `push_subs`, rate-limit/cache, email digest, PWA gating | yes (with `docs/design/11`) | with `docs/design/11` |
| **docs/design/12** sustainability | `accounts.tier`, `entitlements`, `peers`, `payouts`, `ledger` (recognition-only) | **0012** | with `docs/design/12` |

**Migration numbering:** monetization = **0012**. `docs/design/11` feature tables (`saved_views`, `api_keys`,
...
- [x] Self-host docs: `SOURCE_REPO`/`SOURCE_COMMIT` in `deploy/.env.example` + the deploy README +
      `docs/design/23`; Caddy/Cloudflare route + bypass `/source`.
...
> `federation_sync.ts`; `docs/design/15` T1.3). Account-delete emits signed PII-free find tombstones; peers
...
## Cross-references
- **ADR-1 / ADR-2** reflected in `docs/design/10` (identity & community).
- **ADR-4a / ADR-4b** reflected in `docs/design/11` (API access model + push/email-fallback).
- **migration 0012 + recognition-only + attribution** reflected in `docs/design/12`.
- **ADR-3 / ADR-5** specced here; ADR-5 extends the federation model in `docs/design/06` and joins the
  federation feed family in the next-level roadmap (`docs/design/15-federation-next.md`, T1.3).
