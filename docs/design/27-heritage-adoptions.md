...
**Doc type:** Source-material analysis + adoption proposal. Feeds the roadmap (`docs/design/26`).
...
### A.3 Gaps vs the original vision (fidelity gap-closure — feeds `docs/design/26` Stage 0)
...
### B.1 Hardware — strengthen TNC / modem / rig support (feeds `docs/design/16` + `docs/design/25` P1/P4)
...
- **AGWPE (AGW Packet Engine) TCP interface** — the de-facto multi-port/multi-application TNC server
  ...
  single highest-leverage interop after KISS. (Already flagged in `docs/design/16`.)
- **PC/FlexNet driver + autorouting** — GP ran natively over **PC/FlexNet**, where you type a
  ...
  (`docs/design/25` P4): connect by alias, let the node route, and surface the learned-route tree in the UI.
...
### B.2 Terminal UX — adopt into `docs/design/25` P1 (the browser packet terminal)
...
- **ANSI colour terminal** — GP rendered remote **ANSI** colour/box-drawing (BBS menus came alive).
  ...
  our own Cogmind-themed BBS render as intended. (Token-mapped to the active theme — see `docs/design/24`.)
...
> This section is the design seed; `docs/design/26` carries it as a roadmap item.

---

## Adoption summary → roadmap mapping (`docs/design/26`)
...
- **Stage 1 (packet) gains** the GP **hardware** adoptions — the **GPLSL-style driver abstraction**
  ...
  paths + node tree** (P4), and **Multiport** per-port MYCALL — into P1/P4 + `docs/design/16`; and the GP
