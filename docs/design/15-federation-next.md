...
rules. Resolves docs/design/06 open questions **#2 (namespace authority)**, **#3 (open vs peer-approved)**,
**#4 (position privacy)**; integrates ADR-5 tombstones from `docs/design/14`.
...
### T1.1 Peer trust tiers + quarantine  *(resolves docs/design/06 #3)*
...
### T1.2 Corroboration quorum + hardening  *(resolves docs/design/06 #4)*
...
- **Privacy coarsening** *(done; docs/design/06 #4)* — the **asker** snaps its query center to a **grid square**
...
GDPR deletes and removed caches propagate. Full spec in `docs/design/14`; here it joins the feed family:
...
### T2.3 Joining from behind NAT / firewall (outbound-only peers)  *(serves docs/design/06 §6 #4 replication, §9 mesh)*
...
(`docs/design/23` Topology 1). (1) push-to-hub shipped: `POST /federation/submit` (`federation_sync.ts`) + the
...
- **(0) Reverse tunnel — supported today, zero new code.** A free **Cloudflare Tunnel** (`cloudflared`,
  already the `docs/design/23` Topology 1 recipe), Tailscale Funnel, or ngrok gives the box a stable public
...
- **(2) Gateway-as-relay / rendezvous — full solution.** The NAT'd peer opens a **persistent outbound
  ...
  same **"gateway-as-cloud-relay (ECHOCAT pattern; no port-forward)"** already reserved in `docs/design/20` for
...
- *Worth:* makes the "operator-owned box, anywhere" promise real for the home/off-the-shelf case the
  ...
  a directly-served one (Tier still set by `verify.ts` + quorum), consistent with `docs/design/22`'s "transport
...
Surface mirrored caches (already in `remote_caches`) through the public read API (`docs/design/11` / ADR-4a)
...
### T4.2 Instance registry / namespace authority  *(resolves docs/design/06 #2)*
...
`docs/design/19`) is what makes a peer directly addressable *as a network node* — so the platform/any peer
...
(ties to `docs/design/19`) and a DNS-`TXT` anchor as an alternative source.
