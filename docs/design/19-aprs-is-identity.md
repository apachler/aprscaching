...
`BBS_CALL` defaults to `APRSCG`). Build against the cost rules and `docs/design/16` (gated TX) + `docs/design/15`
(federation registry).
...
> message-challenge) — see `docs/design/10` — never by the passcode.
...
| **User, direct** (browser/RF, `docs/design/16`) | the **user's** call | the user's call | auto-computed passcode *or* LoTW-TLS | verification + explicit opt-in (H5) |
...
- **Directly addressable** = the peer *is* `CALL-10`: users send APRS **messages to** it, it **acks
  from** it, and the **federation instance registry** (`docs/design/15` T4.2) binds `instance ↔ {url, key,
...
**B — Direct user→APRS-IS / user→RF (browser-direct, `docs/design/16` H5).** The user's own client logs in as
...
`account_callsigns`, `docs/design/10`).
...
- **Federation descriptor** — add `aprsCall` to `handleWellKnown` output; optional `aprs_call` column
  on `fed_peers` lands with the `docs/design/15` registry work (no migration needed before then).
...
- **P5 — direct user TX (path B):** with `docs/design/16` H5 (auto-passcode/LoTW, verified, opt-in).
- Registry `aprsCall` binding lands with `docs/design/15` T4.2.
