...
| **Day/night terminator + bearing arc** *(from POTACAT, `docs/design/20`)* | A day/night terminator overlay, a great-circle **bearing arc** from your home QTH to a selected cache/station, and a home-QTH marker. | web (MapLibre layers; client geo math) | M2 |
...
| **GPX/KML/ADIF export + public API** | GPX export of caches (for GPS devices) + KML of tracks + **ADIF** export of finds/activations (for standard logbooks — Log4OM/N1MM/DXLab; *from POTACAT, `docs/design/20`*); a public **read API** (caches in bbox, cache detail, station track) à la the aprs.fi API. | edge (Worker endpoints generate GPX/KML/ADIF/JSON; rate-limited + keys) | GPX M3 · API M3–M4 |
| **Proximity / new-cache / watchlist alerts** | In-field geofence prompt already exists (M2). Add **background** alerts: "new cache published near you," "a cache you watch had a find/DNF," and **"a watched callsign is active/heard/near a cache"** (*watchlist, from POTACAT, `docs/design/20`*). | web (Service Worker + Push API) · edge (DO/cron dispatch) | M4 |
...
**Push note (decided — `docs/design/14` ADR-4b):** push is **permission- and PWA-install-gated** (iOS needs
...
```
**Access model (decided — `docs/design/14` ADR-4a):** read-only and **free**. Anonymous access is
