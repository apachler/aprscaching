...
`docs/design/16-rf-hardware-interfacing.md`, `docs/design/24-cogmind-theme.md`, `docs/design/15-federation-next.md`,
...
5. **One product, two skins:** the *modern* theme renders a clean web UI; switching to the **Cogmind
   theme (`docs/design/24`) transforms the same surfaces into a late-90s green-screen packet experience** —
...
- **Pure, testable protocol cores.** The AX.25 connected-mode state machine and NET/ROM live in
  ...
  link-layer right without a radio. The transports (`docs/design/16` H1–H6 Web Serial/BLE/audio, and the
...
- **Trust model unchanged (`workers/gateway/src/verify.ts`, `docs/design/22`).** Connected-mode RX is still
...
```
                ...
                └───────────────▲───────────────────────────────────────────────▲─────────────────┘
   transports (docs/design/16)         │                                                │     services
   ...
   KISS-over-TCP / AXIP ────────┤  (operator-local; ingest-locality)            ├─ Digipeater
                                │                                                ├─ NET/ROM Node
   gateway relay (docs/design/20) ─────┘  remote box ⇄ browser                         └─ FBB BBS + forwarder
                                                                                        │
...
- **Transports**: the existing `docs/design/16` browser pipes (H1 serial, H2 BLE, H4 audio) and the ingest
...
- **Retro shell = Cogmind theme + a "terminal" interaction layer.** `docs/design/24` already makes Cogmind a
  full-app token theme (phosphor palette, CP437/box-drawing, monospace, "all info visible" HUD). This
  proposal **extends `docs/design/24`** with a `data-shell="terminal"` mode (auto-enabled under the Cogmind
...
- **Bulletin federation (just shipped, `docs/design/15`/this repo).** It already floods BID-deduped
  ...
  **`rf-fbb`** (connected-mode FBB protocol over RF) and **`axip`** (`docs/design/22`, reserved). One
...
- **Remote box relay (`docs/design/20`).** Lets the browser terminal/admin reach the operator's box (node,
...
- **Don't boil the ocean.** Out of scope (for now): full WP flooding parity, NTS/traffic handling
  ...
  `docs/design/22`). Reserve seams; ship the spine.
