...
across `docs/design/01` (Phase 8), `docs/design/08` (RF modes backlog), and `apps/ingest`. Two complementary paths,
...
**Path B — browser-direct hardware (PLANNED — `docs/design/01` Phase 8, "stretch").** The SPA reaches local
...
|---|---|---|
| KISS TNC over **USB serial** | ✅ Chromium **desktop** (Android emerging) | Web Serial — desktop solid; Android only from Chrome 148 beta (Apr 2026), BT-RFCOMM-first, USB-serial limited ~2026Q2. **No Safari/Firefox/iOS** → companion (`docs/design/21`) |
| KISS TNC over **Bluetooth LE** | ✅ Chromium desktop/Android | Web Bluetooth + **BLE-KISS API** (below); **not iOS Safari** (iOS BLE = the Capacitor companion, `docs/design/21`) |
...
4. **FX.25 / IL2P** (NinoTNC, Direwolf) — decode-yield for stronger Tier-A (`docs/design/08` BL-01); RX-side
...
- A browser-direct or box **receiver is another independent IGate** → more independent receptions per
  beacon → more finds reach **Tier A** (the federation corroboration in `docs/design/15` benefits directly).
...
- **H6 — Web Serial CAT (one-click tune)** *(from POTACAT, `docs/design/20`; full design in `docs/design/21`)*: drive
  ...
  iOS/non-Chromium go through the **Hamlib companion** (`docs/design/21`), not the browser.
