...
The workbench is the operator side of the app — a real APRS bench. It is revealed progressively, so the
cacher never sees it, and each app opens into its own surface.

## Decoder & live stations

The pure decoder (`@aprsweb/aprs`) turns any raw frame into typed data: uncompressed, base-91 compressed, and
...
The workbench is extensible: third parties can ship **tool plugins** that add commands, decoders,
...
- **Remote control** — drive your own ingest box from the workbench without any inbound port; the box pulls
