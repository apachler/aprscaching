...
`.claude/rules/{ui-ux,css}.md`, the cost rules, and `docs/16` (RF hardware) + `docs/19` (APRS-IS id).
...
Today our POTA/SOTA/WWFF data are **static** imported caches (`docs/11`/`docs/03`). POTACAT's core
...
- **Control surface (from the web app → gateway → the user's box):** trigger a beacon, send an APRS
  message, manage TX / IGate / digi state, view live RX, run a WX beacon (`docs/17`).
...
- **Gating (non-negotiable):** every TX-capable command is gated by **control-verification + explicit
  opt-in** (`docs/16` H5, `docs/19`); the box only executes commands signed for the callsign it's
...
- **Federation note:** this is *operator→own-box* control, distinct from federation peer messaging
  (`docs/19` `aprsCall`). Don't conflate the command channel with the public APRS service identity.
...
```sql
-- watched callsigns (powers the alert in §4 of docs/11 / ADR-4b)
...
Small extensions tracked in their home docs: **Web Serial CAT one-click tune** → `docs/16` (new
