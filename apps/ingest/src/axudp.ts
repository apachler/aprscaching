...
/**
 * PURE: encode an AX.25 frame into the AXUDP wire payload for TX — the AX.25 frame followed by the
 * RFC 1226 CRC trailer (what BPQAXIP and ax25ipd expect in the UDP datagram body; see
 * `@aprscaching/ax25` axip-crc). Symmetric with `axudpToPacket`'s decode; used by `AxudpPort.sendFrame`.
 */
...
/**
 * PURE: normalize one AXUDP datagram (an AX.25 frame over UDP, with or without the RFC 1226 CRC
 * trailer) into a Tier-C ingest Packet, or null if it isn't a decodable UI/APRS frame. The trust decision lives here and is deliberately fixed:
 * `heardVia: "aprs_is"` + `port: "axudp"` → the gateway's provenance derivation stamps
 * `firstPartyAttested = false`, so a tunnelled frame can NEVER reach Tier A ("transport ≠ trust").
...
export function axudpToPacket(datagram: Uint8Array, nowS = Math.floor(Date.now() / 1000)): Packet | null {
  const f = decodeAx25(stripAxipCrc(datagram));
  if (!f) return null;
  return {
    src: f.src,
    dst: f.dst,
...
/**
 * AXUDP as a bidirectional KISS-equivalent PORT: the same raw AX.25 frames a KISS TNC
...
export class AxudpPort {
  ...
  start() {
    ...
    s.on("error", (e: NodeJS.ErrnoException) => {
      // an unresolvable/absent peer re-errors on every send — log each distinct error once a minute
      const last = this.lastErrLog.get(e.message) ?? 0;
      if (Date.now() - last > 60_000) {
        this.lastErrLog.set(e.message, Date.now());
        console.error("[axudp] socket error:", e.message);
      }
      if (e.code === "EADDRINUSE") setTimeout(() => s.bind(this.o.port, this.o.bind), 5000).unref?.(); // retry the bind once the address frees up
    });
  ...
  offRaw(cb: (b: Uint8Array) => void): void {
    const i = this.rawCbs.indexOf(cb);
    if (i >= 0) this.rawCbs.splice(i, 1);
  }
