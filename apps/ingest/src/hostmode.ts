...
export interface HostmodeOpts { host: string; port: number; mycall?: string; radioPort?: number }
...
export class HostmodeTnc {
  ...
  constructor(private o: HostmodeOpts, private h: HostmodeHandlers) {}

  start() { this.connect(); }

  private send(bytes: Uint8Array) { try { this.sock?.write(Buffer.from(bytes)); } catch { /* link down */ } }
  ...
  private connect() {
    ...
    s.on("connect", () => {
      ...
      this.poll = setInterval(() => { if (this.connected) this.send(hostmodeCommand(0, "G")); }, 500);
    ...
    const down = () => { this.connected = false; if (this.poll) clearInterval(this.poll); };
    s.on("error", () => { down(); console.log("[hostmode] disconnected, retrying…"); });
  ...
  private onEvent(ev: HostmodeEvent): void {
    if (ev.type === 4) { this.pendingHeader = ev.text; return; }       // header, info follows separately
    if (ev.type === 5) { this.emitMonitor(ev.header, ev.info); return; }
    if (ev.type === 6 && this.pendingHeader) { this.emitMonitor(this.pendingHeader, ev.info); this.pendingHeader = null; }
  }

  private emitMonitor(header: string, info: Uint8Array): void {
    if (!header.includes(">")) return;                                  // not a parseable addr header
    let body = ""; for (const b of info) body += String.fromCharCode(b);
    const f = parseTNC2(`${header.trim()}:${body}`);
    if (!f) return;
    this.h.onFrame?.(f);
    this.h.onPacket({
      src: f.src, dst: f.dst, path: f.path, payload: f.payload,
      kind: "other", heardVia: "rf", port: "hostmode",
