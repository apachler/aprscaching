...
const STYLE: string | StyleSpecification =
  import.meta.env.VITE_BASEMAP === "offline" ? buildGraticuleStyle() : "https://demotiles.maplibre.org/style.json";
...
const TOUR_STEPS: TourStep[] = [
  {
    title: "Quick tour",
    body: "Guided tour coming soon. For now you're browsing in read-only mode — explore the map and caches freely. Sign in to log finds, hide caches, and unlock the workbench.",
  },
];

/**
 * Platform — the signed-in / explore workbench: the MapLibre map plus every panel. Lazily imported by
    ...
    for (const c of shown) {
      ...
      if (existing) {
        ...
        if (c.type !== "aprs_living") {
          ...
          if (span) span.textContent = cogmind ? meta.cog : meta.glyph;
        }
      } else {
