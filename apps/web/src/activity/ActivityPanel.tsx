  ...
  return (
    <Panel title="Activity" onClose={props.onClose}>
      ...
      {feed.error && feed.items.length === 0 ? (
        ...
        <EmptyState>No recent activity here — be the first to log a find.</EmptyState>
      ) : (
        <ul className="logs">
          {feed.items.map((a) => (
            <li key={a.id}>
      ...
      <ol className="board">
        {top.map((e) => (
          <li key={e.loggerCall}>
            ...
            <span className="mono flex-1">{e.loggerCall}</span>
          ...
          <ol className="board">
            {corr.map((c) => (
              <li key={c.igate}>
                ...
                <span className="mono flex-1">{c.igate}</span>
                <strong>{c.corroborations}</strong>&nbsp;<span className="muted">✓</span>
              </li>
            ))}
          </ol>
        </>
