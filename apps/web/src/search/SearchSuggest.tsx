...
export function SearchSuggest(props: {
  ...
  function pick(h: Hit) {
    setOpen(false);
    if (h.kind === "cache") props.onPickCache(h);
    else props.onPickStation(h);
  }
          ...
          {hits.map((h, i) => (
            ...
            >
              {h.kind === "cache" ? (
                ...
                <>
                  <span className="so-kind stn">RF</span>
                  <span className="so-title mono">{h.callsign}</span>
                  {h.comment && <span className="so-meta">{h.comment}</span>}
                </>
              )}
            </li>
          ))}
      ...
      {showEmpty && (
        <ul className="search-pop" role="listbox" aria-label="Search results">
          <li className="search-empty">
            No matches — try a grid like <span className="mono">JN77</span> or “lat, lon”.
          </li>
        </ul>
      )}
    </div>
