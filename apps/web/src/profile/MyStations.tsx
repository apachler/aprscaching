...
/**
 * Settings → My stations (docs/13 M5). Manage the operator's own stations — a home weather PWS, a
 * mountain-top digipeater / igate / node — each with its own callsign+SSID, explicit location,
 * description and roles. Weather-capable stations carry their own PWS push key. Paginated (docs/11).
 */
export function MyStations(props: { callsign: string }) {
  const toast = useToast();
  const stations = usePaged((cursor) => listMyStations(cursor).then((r) => ({ items: r.stations, nextCursor: r.nextCursor, hasMore: r.hasMore })), [props.callsign]);
  const base = props.callsign.toUpperCase().split("-")[0] ?? "";
  const [draft, setDraft] = useState<StationInput>({ callsign: base ? `${base}-` : "", roles: [] });

  async function add() {
    try { await createStation({ ...draft, callsign: draft.callsign?.trim().toUpperCase() }); setDraft({ callsign: base ? `${base}-` : "", roles: [] }); toast("Station added"); stations.reload(); }
  ...
  return (
    <>
      ...
      <div className="row end mt-2"><button className="primary" onClick={add} disabled={!draft.callsign || draft.callsign.endsWith("-")}>Add station</button></div>
    </>
  );
}
...
function StationCard(props: { station: OperatedStation; onChanged: () => void }) {
  ...
  async function save() {
    setBusy(true);
    try { await updateStation(props.station.id, edit); toast("Station saved"); props.onChanged(); setOpen(false); }
    catch (e) { toast((e as Error).message); } finally { setBusy(false); }
  }
  async function remove() {
    if (!confirm(`Remove ${props.station.callsign}? Its weather push key (if any) is revoked.`)) return;
    try { await deleteStation(props.station.id); toast("Station removed"); props.onChanged(); }
    catch (e) { toast((e as Error).message); }
  }
  ...
  return (
    <div className="station-card">
      <button className="station-head" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        ...
        <span className="badges">{props.station.roles.map((r) => <Badge key={r}>{ROLE_LABEL[r]}</Badge>)}</span>
...
function StationFields(props: { value: StationInput; onChange: (v: StationInput) => void; callsignEditable?: boolean }) {
  const v = props.value;
  ...
  return (
    <>
      {props.callsignEditable && (
        <label>Callsign <span className="muted">(with SSID, e.g. OE8APR-1)</span>
          <input className="mono" value={v.callsign ?? ""} maxLength={11} placeholder="OE8APR-1" onChange={(e) => set({ callsign: e.target.value.toUpperCase() })} /></label>
      )}
      <div className="row gap-2">
        <label className="flex-1">Latitude <input className="mono" inputMode="decimal" value={v.lat ?? ""} placeholder="47.07" onChange={(e) => set({ lat: numOrNull(e.target.value) })} /></label>
        <label className="flex-1">Longitude <input className="mono" inputMode="decimal" value={v.lon ?? ""} placeholder="15.42" onChange={(e) => set({ lon: numOrNull(e.target.value) })} /></label>
      </div>
      <label>Description <input value={v.description ?? ""} maxLength={280} placeholder="Schöckl summit digipeater" onChange={(e) => set({ description: e.target.value })} /></label>
      <fieldset className="roles">
        <legend>Roles</legend>
        {STATION_ROLES.map((r) => (
          <label key={r} className="role-check">
            <input type="checkbox" checked={(v.roles ?? []).includes(r)} onChange={() => toggleRole(r)} /> {ROLE_LABEL[r]}
          </label>
        ))}
      </fieldset>
    </>
...
function StationWxKeyPanel(props: { stationId: number }) {
  ...
  const [busy, setBusy] = useState(false);
  useEffect(() => { getStationWxKey(props.stationId).then(setInfo).catch(() => {}); }, [props.stationId]);
  async function issue() {
    if (info?.key && !confirm("Issue a new key? The station stops reporting until you update its URL.")) return;
    setBusy(true);
    try { setInfo(await issueStationWxKey(props.stationId)); toast(info?.key ? "New key issued" : "Weather push enabled"); }
    catch (e) { toast((e as Error).message); } finally { setBusy(false); }
  }
  const copy = (val: string, what: string) => { navigator.clipboard?.writeText(val); toast(`${what} copied`); };
  return (
    <div className="wx-key">
      <h5>Weather push</h5>
      {!info?.key ? (
        <div className="row end"><button className="primary" onClick={issue} disabled={busy}>Enable weather push</button></div>
      ) : (
        <>
          <label>Ecowitt — custom server path
            <span className="copyrow"><input className="mono" readOnly value={info.ecowittPath ?? ""} onFocus={(e) => e.currentTarget.select()} />
              <button className="iconbtn" aria-label="Copy Ecowitt URL" onClick={() => copy(info.ecowittPath ?? "", "Ecowitt URL")}>copy</button></span></label>
...
function toInput(s: OperatedStation): StationInput {
  return { callsign: s.callsign, lat: s.lat, lon: s.lon, symbol: s.symbol, description: s.description ?? "", roles: s.roles };
}
