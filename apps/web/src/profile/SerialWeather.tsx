...
import { decodeUltimeter, type UltimeterReading } from "@aprscaching/aprs";
...
export function SerialWeather(props: { wxKey: string | null }) {
  ...
  async function maybeSubmit(r: UltimeterReading) {
    ...
    try {
      await submitWxReading(key, r);
      setSentAt(Math.floor(t / 1000));
    } catch (e) {
      console.warn(e); /* retry on the next packet */
    }
  }
  ...
  if (!supported)
    return (
      <p className="muted fine">
        Browser-direct weather needs Web Serial — use Chromium on desktop (no Safari/Firefox/iOS). Your station can
        still push via the URLs above.
      </p>
    );
  ...
  return (
    <div className="serial-wx">
      <p className="muted fine">
        Plug a Peet Bros / Ultimeter station into USB and read it directly in the browser — no daemon. Decoded readings
        post to your station once a minute.
      </p>
      <div className="row">
        <label>
          Baud{" "}
          <select value={baud} disabled={connected} onChange={(e) => setBaud(+e.target.value)}>
            {BAUDS.map((b) => (
