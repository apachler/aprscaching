...
const SRC = {
  grid: "mt-grid",
  rings: "mt-rings",
  rulerLine: "mt-ruler-line",
  rulerPts: "mt-ruler-pts",
  night: "mt-night",
  arc: "mt-arc",
  homePt: "mt-home",
};
  ...
  const feats: GeoJSON.Feature[] = radii.map((r) => ({
    ...
    geometry: {
      ...
      coordinates: Array.from({ length: 65 }, (_, i) => {
        const p = destinationPoint(lat, lon, (i * 360) / 64, r);
...
export function MapTools(props: {
  ...
  useEffect(() => {
    ...
    const redraw = () => {
      ...
      if (stateRef.current.rings) {
        ...
        setRadii(rs);
        setData(m, SRC.rings, ringFC(c.lat, c.lng, rs));
      } else {
        setRadii([]);
        setData(m, SRC.rings, EMPTY);
      }
    };
    redraw();
    m.on("moveend", redraw);
  ...
  useEffect(() => {
    ...
    const onClick = (e: maplibregl.MapMouseEvent) => {
      ...
      setPts(next);
      setData(m, SRC.rulerPts, {
        type: "FeatureCollection",
        features: next.map((p) => ({ type: "Feature", properties: {}, geometry: { type: "Point", coordinates: p } })),
      });
      setData(
        m,
        SRC.rulerLine,
        next.length === 2
          ? {
              type: "FeatureCollection",
              features: [{ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: next } }],
            }
          : EMPTY,
      );
    };
  ...
  function clearRuler() {
    ...
    setPts([]);
    if (m) {
      setData(m, SRC.rulerLine, EMPTY);
      setData(m, SRC.rulerPts, EMPTY);
    }
  }
  ...
  useEffect(() => {
    ...
    if (!m) return;
    if (!arc || !home || !target) {
      setData(m, SRC.arc, EMPTY);
      setData(m, SRC.homePt, EMPTY);
      return;
    }
    if (!m.isStyleLoaded()) return;
  ...
  const measure =
    pts.length === 2
      ? {
          d: haversineMeters(pts[0]![1], pts[0]![0], pts[1]![1], pts[1]![0]),
          b: initialBearing(pts[0]![1], pts[0]![0], pts[1]![1], pts[1]![0]),
        }
      : null;
  const arcOk = !!(home && target);
  const arcMeasure = arcOk
    ? {
        d: haversineMeters(home![0], home![1], target![0], target![1]),
        b: initialBearing(home![0], home![1], target![0], target![1]),
      }
    : null;
    ...
    <div className="maptools">
      ...
      {arc && (
        <div className="maptools-legend">
          {arcMeasure ? (
            <>
              QTH→cache: <strong>{fmt.distance(arcMeasure.d)}</strong> · {arcMeasure.b.toFixed(0)}°
            </>
          ) : !home ? (
            "add a home locator in your profile"
          ) : (
            "select a cache to draw the bearing"
          )}
        </div>
      )}
    </div>
