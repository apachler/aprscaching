...
export function BbsPanel(props: { callsign: string; onClose: () => void }) {
  ...
  const load = useCallback(() => {
    ...
    if (signedIn) {
      getBbsInbox(props.callsign)
        .then((r) => setInbox(r.messages))
        .catch(fail);
      getBbsSent(props.callsign)
        .then((r) => setSent(r.messages))
        .catch(fail);
    }
    getBulletins()
      .then((r) => setBulletins(r.bulletins))
      .catch(fail);
  ...
  function openMessage(m: BbsMessage) {
    ...
    if (m.type !== "B" && m.readAt == null) {
      ...
      void markBbsRead(m.id).catch(() => {
        /* a re-load will reconcile */
      });
    }
  }
  ...
  function reply(m: BbsMessage) {
    ...
    setType(m.type === "B" ? "B" : "P");
    setReplyTo(m.id); // thread the reply (SR)
    setTab("compose");
    setMsg(null);
  }
  ...
  async function send() {
    ...
    try {
      const r = await postBbsMessage({
        fromCall: props.callsign,
        toCall: to.trim().toUpperCase(),
      ...
      setMsg(
        r.type === "B"
          ? "Bulletin posted to the network."
          : r.type === "T"
            ? "Traffic stored."
            : "Held — it'll be delivered when the station is next heard.",
      );
      setBody("");
      setTo("");
      setSubject("");
    ...
    return (
      <article className="bbs-read">
        ...
        <dl className="bbs-hdr">
          <div>
            <dt>From</dt>
            <dd className="mono">{selected.fromCall}</dd>
          </div>
          <div>
            <dt>To</dt>
            <dd className="mono">{selected.toCall}</dd>
          </div>
          <div>
            <dt>BID</dt>
            <dd className="mono">{selected.bid}</dd>
          </div>
          <div>
            <dt>Date</dt>
            <dd>{fmt.dateTime(selected.postedAt)}</dd>
          </div>
        </dl>
        {tree.length > 1 ? (
          <>
            ...
            <div className="bbs-thread">
              {tree.map(({ msg: t, depth }) => (
                <div
                  key={t.id}
                  className={`bbs-node depth-${Math.min(depth, 6)}${t.id === selected.id ? " on" : ""}`}
                  onClick={() => setSelected(t)}
                  onKeyDown={rowKey(() => setSelected(t))}
                  role="button"
                  tabIndex={0}
                >
                  <div className="bbs-msg">
                    <div className="bbs-msg-h">
                      {depth > 0 && (
                        <span className="bbs-reply-mark" aria-hidden="true">
                          ↳
                        </span>
                      )}
