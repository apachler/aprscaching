...
export function AccountSettings(props: { session: Session; onSignIn: () => void }) {
  ...
  async function setActive(cs: string) {
    ...
    try {
      ...
      setMsg(`Now operating as ${cs}.`);
      refresh();
      await reload();
    } catch (e) {
      setMsg((e as Error).message.replace(/^.*?: /, ""));
    } finally {
      setBusy(false);
    }
  }
  async function startVerify(cs: string) {
    ...
    setMsg(null);
    try {
      await startAprsVerify(cs);
      setVerifyingCall(cs);
      setCode("");
      setMsg(`Sent a code to ${cs} over APRS — read it on your radio and enter it.`);
    } catch (e) {
  ...
  async function confirm(cs: string) {
    ...
    try {
      ...
      setMsg("That code didn't match. Check your radio and try again.");
    } finally {
      setBusy(false);
    }
  }
  async function addNew() {
    ...
    if (n.length < 3) {
      setMsg("Enter a callsign to add.");
      return;
    }
    setBusy(true);
    setMsg(null);
    try {
      await addCallsign(n);
      setNewCs("");
      setMsg(`Added ${n} — verify it below to enable announce + leaderboard credit.`);
      await reload();
    } catch (e) {
      setMsg((e as Error).message.replace(/^.*?: /, ""));
    } finally {
    ...
    <Group title="Account" status={active}>
          ...
          <li key={c.callsign} className="setrow">
            ...
            <div className="setrow-c">
              {c.verified ? (
                ...
                <button className="link" disabled={busy} onClick={() => startVerify(c.callsign)}>
                  verify
                </button>
              )}
              {!c.active && (
      ...
      {verifyingCall && (
        <div className="verify-code">
          <label className="m-0">
            Code sent to <span className="mono">{verifyingCall}</span>
            <input
              value={code}
          ...
          <div className="row end mt-2">
            <button onClick={() => setVerifyingCall(null)}>Cancel</button>
            <button
              className="primary"
              disabled={busy || code.trim().length < 4}
              onClick={() => confirm(verifyingCall)}
            >
