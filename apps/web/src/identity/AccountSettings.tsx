...
export function AccountSettings(props: { session: Session; onSignIn: () => void }) {
  ...
  async function startVerify(cs: string) {
    ...
    try {
      ...
      setVerifyingCall(cs);
      setCode("");
      setMsg({ text: `Sent a code to ${cs} over APRS — read it on your radio and enter it.`, kind: "ok" });
    } catch (e) {
  ...
  async function confirm(cs: string) {
    ...
    try {
      ...
      if (r.verified) {
        ...
        setMsg({ text: `${cs} verified ✓`, kind: "ok" });
        if (cs === active) refresh();
        await reload();
      } else setMsg({ text: "That code didn't match. Check your radio and try again.", kind: "error" });
    } catch {
      setMsg({ text: "That code didn't match. Check your radio and try again.", kind: "error" });
    } finally {
      setBusy(false);
    }
  }
