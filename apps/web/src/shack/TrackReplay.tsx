...
export function TrackReplay(props: { map: maplibregl.Map | null; callsign: string }) {
  ...
  useEffect(() => {
    ...
    setPlaying(false);
    setErr(null);
    setData(SRC.line, EMPTY);
    setData(SRC.pts, EMPTY);
    setData(SRC.head, EMPTY);
    ...
    setData(
      ...
      h
        ? {
            ...
            features: [{ type: "Feature", properties: {}, geometry: { type: "Point", coordinates: [h.lon, h.lat] } }],
          }
        : EMPTY,
    );
  }
  ...
  async function load() {
    ...
    try {
      ...
      if (day) {
        from = Date.parse(`${day}T00:00:00Z`) / 1000;
        to = from + 86400;
      } else {
        to = Date.now() / 1000;
        from = to - days * 86400;
              ...
              features: [
                {
                  type: "Feature",
                  properties: {},
                  geometry: { type: "LineString", coordinates: p.map((x) => [x.lon, x.lat]) },
                },
              ],
            }
          : EMPTY,
  ...
  useEffect(() => {
    if (pts.length) drawHead(pts, idx);
  ...
  useEffect(() => {
    if (playing && pts.length && idx >= pts.length - 1) setPlaying(false);
  }, [idx, playing, pts.length]);
  ...
  return (
    <div className="trackreplay">
      ...
      {!err && pts.length > 0 && (
        <>
          <div className="tr-meta">
            <span>
              <strong>{pts.length}</strong> fixes
            </span>
            <span className="tr-legend">
              <i className="rf" /> RF <i className="app" /> app <i className="is" /> IS
            </span>
          </div>
      ...
      {!err && !loading && pts.length === 0 && (
        <div className="muted tr-empty">Pick a window and Load to see the track.</div>
      )}
    </div>
  );
}
