...
import { catSetFrequency, catSetMode, type CatRig } from "@aprscaching/aprs";

/**
 * cat.ts — browser-direct CAT rig control over Web Serial. A single shared controller so
 * any surface (the Shack rig panel, a spot card) can one-click tune the connected radio. Tuning
 * is RX-side (set VFO frequency/mode) — not transmitting — so it does not require callsign
...
class CatController {
  ...
  async tune(hz: number, mode?: string | null): Promise<void> {
    ...
    try {
      ...
      if (mode) {
        const mb = catSetMode(this.profile.rig, mode, opts);
        if (mb) await w.write(mb);
      }
    } finally {
      w.releaseLock();
    }
